...
"poetry install" из корневой директории.
...
### Выполнение миграции таблиц базы данных.
Для выполнения миграции таблиц базы данных, необходимо выполнить команду "alembic upgrade head" в терминале из корневой директории 2 раза подряд.
## Подключение к Redis.
...
## Заполнение таблиц базы данных данными из таблицы .xlsx.
В приложении есть возможность заполнить таблицы базы данных данными из таблица с расширением .xlsx. Данная таблица(menu.xlsx) расположена в корневой директории.
Чтобы выполнить данную операцию, необходимо указать абсолютный путь до данной таблицы через переменную окружения "PATH_TO_TABLE". Данная переменная окружения расположена в файле ".env" в корневой директории.
Для заполнения таблиц базы данных данными из таблицы .xlsx, необходимо выполнить команду
"python database_entry.py" в терминале из корневой директории.
## Отчищение таблиц базы данных.
Для отчищения всех таблиц базы данных, необходимо выполнить команду "python clear_database.py"
